...
hybridElimination(const HybridGaussianFactorGraph &factors,
  ...
  using EliminationPair = std::pair<std::shared_ptr<GaussianConditional>,
  ...
  if (continuousSeparator.empty()) {
    ...
    const auto discreteFactor =
        std::make_shared<DecisionTreeFactor>(discreteSeparator, fdt);
    ...
    return {std::make_shared<HybridConditional>(gaussianMixture),
            std::make_shared<GaussianMixtureFactor>(
