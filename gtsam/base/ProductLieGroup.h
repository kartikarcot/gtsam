...
public:
  ...
  ProductLieGroup retract(const TangentVector& v, //
      ChartJacobian H1 = std::nullopt, ChartJacobian H2 = std::nullopt) const {
...
public:
  ...
  static ProductLieGroup Expmap(const TangentVector& v, ChartJacobian Hv = std::nullopt) {
    Jacobian1 D_g_first; Jacobian2 D_h_second;
  ...
  static TangentVector Logmap(const ProductLieGroup& p, ChartJacobian Hp = std::nullopt) {
    Jacobian1 D_g_first; Jacobian2 D_h_second;
  ...
  static TangentVector LocalCoordinates(const ProductLieGroup& p, ChartJacobian Hp = std::nullopt) {
    return Logmap(p, Hp);
  }
