...
TEST( OptionalJacobian, Constructors ) {
  Matrix23 fixed;
  Matrix dynamic;
  std::optional<Matrix&> optional(dynamic);
...
void test(OptionalJacobian<2, 3> H = std::nullopt) {
  if (H)
    *H = kTestMatrix;
}
...
//******************************************************************************
void test2(OptionalJacobian<-1,-1> H = std::nullopt) {
  if (H)
...
//******************************************************************************
void test3(double add, OptionalJacobian<2,1> H = std::nullopt) {
  if (H) *H << add + 10, add + 20;
}

// This function calls the above function three times, one for each column
void test4(OptionalJacobian<2, 3> H = std::nullopt) {
  if (H) {
    test3(1, H.cols<1>(0));
    test3(2, H.cols<1>(1));
    test3(3, H.cols<1>(2));
