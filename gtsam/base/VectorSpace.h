...
struct VectorSpaceImpl {
  ...
  static Class Compose(const Class& v1, const Class& v2, ChartJacobian H1 = std::nullopt,
      ChartJacobian H2 = std::nullopt) {
    if (H1) *H1 = Jacobian::Identity();
    if (H2) *H2 = Jacobian::Identity();
    return v1 + v2;
  }

  static Class Between(const Class& v1, const Class& v2, ChartJacobian H1 = std::nullopt,
      ChartJacobian H2 = std::nullopt) {
    if (H1) *H1 = - Jacobian::Identity();
    if (H2) *H2 =   Jacobian::Identity();
    return v2 - v1;
  ...
  static Class Inverse(const Class& v, ChartJacobian H = std::nullopt) {
...
struct ScalarTraits : VectorSpaceImpl<Scalar, 1> {
  ...
  /// @name Lie Group
  /// @{
  static TangentVector Logmap(Scalar m, ChartJacobian H = std::nullopt) {
    if (H) (*H)[0] = 1.0;
    return Local(0, m);
  }

  static Scalar Expmap(const TangentVector& v, ChartJacobian H = std::nullopt) {
    if (H) (*H)[0] = 1.0;
    return v[0];
  }
  /// @}
...
struct traits<Eigen::Matrix<double, M, N, Options, MaxRows, MaxCols> > :
  ...
  /// @name Lie Group
  /// @{
  static TangentVector Logmap(const Fixed& m, ChartJacobian H = std::nullopt) {
    if (H) *H = Jacobian::Identity();
    TangentVector result;
  ...
  static Fixed Expmap(const TangentVector& v, ChartJacobian H = std::nullopt) {
    Fixed m;
    m.setZero();
    if (H) *H = Jacobian::Identity();
...
struct DynamicTraits {
  ...
  /// @name Lie Group
  /// @{
  static TangentVector Logmap(const Dynamic& m, ChartJacobian H = std::nullopt) {
    if (H) *H = Eye(m);
  ...
  static Dynamic Expmap(const TangentVector& /*v*/, ChartJacobian H = std::nullopt) {
    static_cast<void>(H);
    throw std::runtime_error("Expmap not defined for dynamic types");
  }

  static Dynamic Inverse(const Dynamic& m, ChartJacobian H = std::nullopt) {
    if (H) *H = -Eye(m);
    return -m;
  }
