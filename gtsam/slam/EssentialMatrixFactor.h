...
class EssentialMatrixFactor2
 ...
 public:
  ...
  Vector evaluateError(
      ...
      std::optional<Matrix&> DE = std::nullopt,
      std::optional<Matrix&> Dd = std::nullopt) const override {
 ...
 public:
  ...
  Vector evaluateError(
    ...
    if (!DE) {
      ...
      // Evaluate error
      return Base::evaluateError(cameraE, d, std::nullopt, Dd);
    } else {
...
class EssentialMatrixFactor4
 ...
 public:
  ...
  Vector evaluateError(
    ...
    Point2 cA = K.calibrate(pA_, H2 ? &cA_H_K : 0, std::nullopt);
    Point2 cB = K.calibrate(pB_, H2 ? &cB_H_K : 0, std::nullopt);
