...
TEST(Unit3, rotate) {
  ...
  // Use numerical derivatives to calculate the expected Jacobian
  {
    ...
    R.rotate(p, actualH, std::nullopt);
    EXPECT(assert_equal(expectedH, actualH, 1e-5));
  }
  {
    expectedH = numericalDerivative22(rotate_, R, p);
    R.rotate(p, std::nullopt, actualH);
    EXPECT(assert_equal(expectedH, actualH, 1e-5));
  }
}
...
TEST(Unit3, unrotate) {
  ...
  // Use numerical derivatives to calculate the expected Jacobian
  {
    ...
    R.unrotate(p, actualH, std::nullopt);
    EXPECT(assert_equal(expectedH, actualH, 1e-5));
  }
  {
    expectedH = numericalDerivative22(unrotate_, R, p);
    R.unrotate(p, std::nullopt, actualH);
    EXPECT(assert_equal(expectedH, actualH, 1e-5));
  }
}
...
TEST(Unit3, error) {
  ...
  // Use numerical derivatives to calculate the expected Jacobian
  {
    expected = numericalDerivative11<Vector2,Unit3>(
        std::bind(&Unit3::error, &p, std::placeholders::_1, std::nullopt), q);
    p.error(q, actual);
    EXPECT(assert_equal(expected.transpose(), actual, 1e-5));
  }
  {
    expected = numericalDerivative11<Vector2,Unit3>(
        std::bind(&Unit3::error, &p, std::placeholders::_1, std::nullopt), r);
    p.error(r, actual);
    EXPECT(assert_equal(expected.transpose(), actual, 1e-5));
  }
}
...
TEST(Unit3, error2) {
  ...
  // Use numerical derivatives to calculate the expected Jacobian
  {
    expected = numericalDerivative21<Vector2, Unit3, Unit3>(
        ...
        p, q);
    p.errorVector(q, actual, std::nullopt);
    EXPECT(assert_equal(expected, actual, 1e-5));
  }
  {
    expected = numericalDerivative21<Vector2, Unit3, Unit3>(
        std::bind(&Unit3::errorVector, std::placeholders::_1,
                  std::placeholders::_2, std::nullopt, std::nullopt),
        p, r);
    p.errorVector(r, actual, std::nullopt);
    EXPECT(assert_equal(expected, actual, 1e-5));
  }
  {
    expected = numericalDerivative22<Vector2, Unit3, Unit3>(
        std::bind(&Unit3::errorVector, std::placeholders::_1,
                  std::placeholders::_2, std::nullopt, std::nullopt),
        p, q);
    p.errorVector(q, std::nullopt, actual);
    EXPECT(assert_equal(expected, actual, 1e-5));
  }
  {
    expected = numericalDerivative22<Vector2, Unit3, Unit3>(
        std::bind(&Unit3::errorVector, std::placeholders::_1,
                  std::placeholders::_2, std::nullopt, std::nullopt),
        p, r);
    p.errorVector(r, std::nullopt, actual);
    EXPECT(assert_equal(expected, actual, 1e-5));
  }
}
...
TEST(Unit3, distance) {
  ...
  // Use numerical derivatives to calculate the expected Jacobian
  {
    expected = numericalGradient<Unit3>(
        std::bind(&Unit3::distance, &p, std::placeholders::_1, std::nullopt), q);
    p.distance(q, actual);
    EXPECT(assert_equal(expected.transpose(), actual, 1e-5));
  }
  {
    expected = numericalGradient<Unit3>(
        std::bind(&Unit3::distance, &p, std::placeholders::_1, std::nullopt), r);
    p.distance(r, actual);
    EXPECT(assert_equal(expected.transpose(), actual, 1e-5));
  }
}
...
TEST(Unit3, basis) {
  ...
  Matrix62 expectedH = numericalDerivative11<Vector6, Unit3>(
      std::bind(BasisTest, std::placeholders::_1, std::nullopt), p);
...
TEST (Unit3, jacobian_retract) {
  ...
  std::function<Unit3(const Vector2&)> f =
      std::bind(&Unit3::retract, p, std::placeholders::_1, std::nullopt);
  {
      Vector2 v (-0.2, 0.1);
      p.retract(v, H);
...
TEST (Unit3, FromPoint3) {
  ...
  Matrix expectedH = numericalDerivative11<Unit3, Point3>(
      std::bind(Unit3::FromPoint3, std::placeholders::_1, std::nullopt), point);
  EXPECT(assert_equal(expectedH, actualH, 1e-5));
}
