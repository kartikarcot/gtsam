...
TEST(Cal3_S2, Duncalibrate1) {
  Matrix25 computed;
  K.uncalibrate(p, computed, std::nullopt);
...
TEST(Cal3_S2, Duncalibrate2) {
  Matrix computed;
  K.uncalibrate(p, std::nullopt, computed);
...
TEST(Cal3_S2, Dcalibrate1) {
  Matrix computed;
  Point2 expected = K.calibrate(p_uv, computed, std::nullopt);
...
TEST(Cal3_S2, Dcalibrate2) {
  Matrix computed;
  Point2 expected = K.calibrate(p_uv, std::nullopt, computed);
