...
public:
  ...
  GTSAM_EXPORT static Pose2 Expmap(const Vector3& xi, ChartJacobian H = std::nullopt);

  ///Log map at identity - return the canonical coordinates \f$ [T_x,T_y,\theta] \f$ of this rotation
  GTSAM_EXPORT static Vector3 Logmap(const Pose2& p, ChartJacobian H = std::nullopt);
  ...
  struct ChartAtOrigin {
	GTSAM_EXPORT static Pose2 Retract(const Vector3& v, ChartJacobian H = std::nullopt);
	GTSAM_EXPORT static Vector3 Local(const Pose2& r, ChartJacobian H = std::nullopt);
  };
  ...
  GTSAM_EXPORT Point2 transformTo(const Point2& point,
      OptionalJacobian<2, 3> Dpose = std::nullopt,
  ...
  GTSAM_EXPORT Rot2 bearing(const Point2& point,
               OptionalJacobian<1, 3> H1=std::nullopt, OptionalJacobian<1, 2> H2=std::nullopt) const;
  ...
  GTSAM_EXPORT Rot2 bearing(const Pose2& pose,
               OptionalJacobian<1, 3> H1=std::nullopt, OptionalJacobian<1, 3> H2=std::nullopt) const;
  ...
  GTSAM_EXPORT double range(const Pose2& point,
      OptionalJacobian<1, 3> H1=std::nullopt,
      OptionalJacobian<1, 3> H2=std::nullopt) const;
