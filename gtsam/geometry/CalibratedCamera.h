...
public:
  ...
  Point2 project2(const Unit3& point,
      OptionalJacobian<2, 6> Dpose = std::nullopt,
  ...
  static Point3 BackprojectFromCamera(const Point2& p, const double depth,
                                      OptionalJacobian<3, 2> Dpoint = std::nullopt,
                                      OptionalJacobian<3, 1> Ddepth = std::nullopt);
...
public:
  ...
  Point3 backproject(const Point2& pn, double depth,
                     ...
                     OptionalJacobian<3, 1> Dresult_ddepth = std::nullopt) const {
  ...
  double range(const CalibratedCamera& camera, //
      OptionalJacobian<1, 6> H1 = std::nullopt, //
      OptionalJacobian<1, 6> H2 = std::nullopt) const {
