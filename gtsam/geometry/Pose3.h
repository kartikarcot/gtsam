...
public:
 ...
 Pose3() : R_(traits<Rot3>::Identity()), t_(traits<Point3>::Identity()) {}
  ...
  static Pose3 Create(const Rot3& R, const Point3& t,
                      OptionalJacobian<6, 3> HR = std::nullopt,
                      OptionalJacobian<6, 3> Ht = std::nullopt);
  ...
  static Pose3 Expmap(const Vector6& xi, OptionalJacobian<6, 6> Hxi = std::nullopt);

  /// Log map at identity - return the canonical coordinates \f$ [R_x,R_y,R_z,T_x,T_y,T_z] \f$ of this rotation
  static Vector6 Logmap(const Pose3& pose, OptionalJacobian<6, 6> Hpose = std::nullopt);
  ...
  Vector6 Adjoint(const Vector6& xi_b,
                  OptionalJacobian<6, 6> H_this = std::nullopt,
                  OptionalJacobian<6, 6> H_xib = std::nullopt) const;
  
  /// The dual version of Adjoint
  Vector6 AdjointTranspose(const Vector6& x,
                           OptionalJacobian<6, 6> H_this = std::nullopt,
                           OptionalJacobian<6, 6> H_x = std::nullopt) const;
  ...
  static Vector6 adjoint(const Vector6& xi, const Vector6& y,
                         OptionalJacobian<6, 6> Hxi = std::nullopt,
                         OptionalJacobian<6, 6> H_y = std::nullopt);
  ...
  struct ChartAtOrigin {
    static Pose3 Retract(const Vector6& xi, ChartJacobian Hxi = std::nullopt);
    static Vector6 Local(const Pose3& pose, ChartJacobian Hpose = std::nullopt);
  };
  ...
  */
  ...
  Point3 transformFrom(const Point3& point, OptionalJacobian<3, 6> Hself =
      std::nullopt, OptionalJacobian<3, 3> Hpoint = std::nullopt) const;
  ...
  /// get rotation
  const Rot3& rotation(OptionalJacobian<3, 6> Hself = std::nullopt) const;

  /// get translation
  const Point3& translation(OptionalJacobian<3, 6> Hself = std::nullopt) const;
  ...
  /** 
    ...
    */
  Pose3 transformPoseFrom(const Pose3& aTb, OptionalJacobian<6, 6> Hself = std::nullopt,
                                            OptionalJacobian<6, 6> HaTb = std::nullopt) const;

  /** 
   *  Assuming self == wTa, takes a pose wTb in world coordinates 
   * and transforms it to local coordinates aTb = inv(wTa) * wTb 
   */
  Pose3 transformPoseTo(const Pose3& wTb, OptionalJacobian<6, 6> Hself = std::nullopt,
                                          OptionalJacobian<6, 6> HwTb = std::nullopt) const;
   ...
   * @return range (double)
   */
  double range(const Point3& point, OptionalJacobian<1, 6> Hself = std::nullopt,
      OptionalJacobian<1, 3> Hpoint = std::nullopt) const;
   ...
   * @return range (double)
   */
  double range(const Pose3& pose, OptionalJacobian<1, 6> Hself = std::nullopt,
      OptionalJacobian<1, 6> Hpose = std::nullopt) const;
   ...
   */
  Unit3 bearing(const Point3& point, OptionalJacobian<2, 6> Hself = std::nullopt,
      OptionalJacobian<2, 3> Hpoint = std::nullopt) const;
   ...
   * @return bearing (Unit3)
   */
  Unit3 bearing(const Pose3& pose, OptionalJacobian<2, 6> Hself = std::nullopt,
      OptionalJacobian<2, 6> Hpose = std::nullopt) const;
   ...
   */
  Pose3 slerp(double t, const Pose3& other, OptionalJacobian<6, 6> Hx = std::nullopt,
                                             OptionalJacobian<6, 6> Hy = std::nullopt) const;
