...
public:
  ...
  static Unit3 FromPoint3(const Point3& point, //
      OptionalJacobian<2, 3> H = std::nullopt);
   ...
   */
  const Matrix32& basis(OptionalJacobian<6, 2> H = std::nullopt) const;

  /// Return skew-symmetric associated with 3D point on unit sphere
  Matrix3 skew() const;

  /// Return unit-norm Point3
  Point3 point3(OptionalJacobian<3, 2> H = std::nullopt) const;

  /// Return unit-norm Vector
  Vector3 unitVector(OptionalJacobian<3, 2> H = std::nullopt) const;
  ...
  double dot(const Unit3& q, OptionalJacobian<1,2> H1 = std::nullopt, //
                             OptionalJacobian<1,2> H2 = std::nullopt) const;

  /// Signed, vector-valued error between two directions
  /// @deprecated, errorVector has the proper derivatives, this confusingly has only the second.
  Vector2 error(const Unit3& q, OptionalJacobian<2, 2> H_q = std::nullopt) const;

  /// Signed, vector-valued error between two directions
  /// NOTE(hayk): This method has zero derivatives if this (p) and q are orthogonal.
  Vector2 errorVector(const Unit3& q, OptionalJacobian<2, 2> H_p = std::nullopt, //
                      OptionalJacobian<2, 2> H_q = std::nullopt) const;

  /// Distance between two directions
  double distance(const Unit3& q, OptionalJacobian<1, 2> H = std::nullopt) const;
  ...
  Unit3 retract(const Vector2& v, OptionalJacobian<2,2> H = std::nullopt) const;
