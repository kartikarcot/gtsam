...
class SO : public LieGroup<SO<N>, internal::DimensionSO(N)> {
 ...
 public:
  ...
  struct ChartAtOrigin {
     ...
     */
    static SO Retract(const TangentVector& xi, ChartJacobian H = std::nullopt);

    /**
     * Inverse of Retract. See note about xi element order in Hat.
     */
    static TangentVector Local(const SO& R, ChartJacobian H = std::nullopt);
  };
   ...
   */
  static SO Expmap(const TangentVector& omega, ChartJacobian H = std::nullopt);
   ...
   */
  static TangentVector Logmap(const SO& R, ChartJacobian H = std::nullopt);
