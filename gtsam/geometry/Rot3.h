...
class GTSAM_EXPORT Rot3 : public LieGroup<Rot3, 3> {
 ...
 public:
    ...
    static Rot3 RzRyRx(double x, double y, double z,
                       OptionalJacobian<3, 1> Hx = std::nullopt,
                       OptionalJacobian<3, 1> Hy = std::nullopt,
                       OptionalJacobian<3, 1> Hz = std::nullopt);

    /// Rotations around Z, Y, then X axes as in http://en.wikipedia.org/wiki/Rotation_matrix, counterclockwise when looking from unchanging axis.
    inline static Rot3 RzRyRx(const Vector& xyz,
                              OptionalJacobian<3, 3> H = std::nullopt) {
      assert(xyz.size() == 3);
      Rot3 out;
      if (H) {
        Vector3 Hx, Hy, Hz;
    ...
    static Rot3 Ypr(double y, double p, double r,
                    OptionalJacobian<3, 1> Hy = std::nullopt,
                    OptionalJacobian<3, 1> Hp = std::nullopt,
                    OptionalJacobian<3, 1> Hr = std::nullopt) {
    ...
    struct CayleyChart {
    static Rot3 Retract(const Vector3& v, OptionalJacobian<3, 3> H = std::nullopt);
    static Vector3 Local(const Rot3& r, OptionalJacobian<3, 3> H = std::nullopt);
    };
     ...
     */
    static Rot3 Expmap(const Vector3& v, OptionalJacobian<3,3> H = std::nullopt) {
     ...
     */
    static Vector3 Logmap(const Rot3& R, OptionalJacobian<3,3> H = std::nullopt);
    ...
    struct ChartAtOrigin {
      static Rot3 Retract(const Vector3& v, ChartJacobian H = std::nullopt);
      static Vector3 Local(const Rot3& r, ChartJacobian H = std::nullopt);
    };
     ...
     */
    Point3 rotate(const Point3& p, OptionalJacobian<3,3> H1 = std::nullopt,
    ...
    Point3 unrotate(const Point3& p, OptionalJacobian<3,3> H1 = std::nullopt,
        OptionalJacobian<3,3> H2=std::nullopt) const;
    ...
    Unit3 rotate(const Unit3& p, OptionalJacobian<2,3> HR = std::nullopt,
        OptionalJacobian<2,2> Hp = std::nullopt) const;

    /// unrotate 3D direction from world frame to rotated coordinate frame
    Unit3 unrotate(const Unit3& p, OptionalJacobian<2,3> HR = std::nullopt,
        OptionalJacobian<2,2> Hp = std::nullopt) const;
     ...
     */
    Vector3 xyz(OptionalJacobian<3, 3> H = std::nullopt) const;

    /**
     * Use RQ to calculate yaw-pitch-roll angle representation
     * @return a vector containing ypr s.t. R = Rot3::Ypr(y,p,r)
     */
    Vector3 ypr(OptionalJacobian<3, 3> H = std::nullopt) const;

    /**
     * Use RQ to calculate roll-pitch-yaw angle representation
     * @return a vector containing rpy s.t. R = Rot3::Ypr(y,p,r)
     */
    Vector3 rpy(OptionalJacobian<3, 3> H = std::nullopt) const;
     ...
     */
    double roll(OptionalJacobian<1, 3> H = std::nullopt) const;
     ...
     * TODO: make this more efficient
     */
    double pitch(OptionalJacobian<1, 3> H = std::nullopt) const;
     ...
     * TODO: make this more efficient
     */
    double yaw(OptionalJacobian<1, 3> H = std::nullopt) const;
  ...
  GTSAM_EXPORT std::pair<Matrix3, Vector3> RQ(
      const Matrix3& A, OptionalJacobian<3, 9> H = std::nullopt);
