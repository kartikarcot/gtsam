...
public:
  ...
  Point2 project(const Point3& pw, OptionalJacobian<2, 6> Dpose = std::nullopt,
      OptionalJacobian<2, 3> Dpoint = std::nullopt,
      OptionalJacobian<2, DimK> Dcal = std::nullopt) const {
    return _project(pw, Dpose, Dpoint, Dcal);
  }

  /// project a 3D point from world coordinates into the image
  Point2 reprojectionError(const Point3& pw, const Point2& measured, OptionalJacobian<2, 6> Dpose = std::nullopt,
      OptionalJacobian<2, 3> Dpoint = std::nullopt,
      OptionalJacobian<2, DimK> Dcal = std::nullopt) const {
  ...
  Point2 project(const Unit3& pw, OptionalJacobian<2, 6> Dpose = std::nullopt,
      OptionalJacobian<2, 2> Dpoint = std::nullopt,
      OptionalJacobian<2, DimK> Dcal = std::nullopt) const {
    return _project(pw, Dpose, Dpoint, Dcal);
  }
  ...
  Point3 backproject(const Point2& p, double depth,
                     OptionalJacobian<3, 6> Dresult_dpose = std::nullopt,
                     OptionalJacobian<3, 2> Dresult_dp = std::nullopt,
                     OptionalJacobian<3, 1> Dresult_ddepth = std::nullopt,
                     OptionalJacobian<3, DimK> Dresult_dcal = std::nullopt) const {
  ...
  double range(const Point3& point,
      OptionalJacobian<1, 6> Dcamera = std::nullopt,
      OptionalJacobian<1, 3> Dpoint = std::nullopt) const {
   ...
   * @return range (double)
   */
  double range(const Pose3& pose, OptionalJacobian<1, 6> Dcamera = std::nullopt,
      OptionalJacobian<1, 6> Dpose = std::nullopt) const {
  ...
  double range(const CalibratedCamera& camera, OptionalJacobian<1, 6> Dcamera =
      std::nullopt, OptionalJacobian<1, 6> Dother = std::nullopt) const {
...
public:
   ...
   *  @param Dpoint is the Jacobian w.r.t. point3
   */
  Point2 project2(const Point3& pw, OptionalJacobian<2, 6> Dpose = std::nullopt,
      OptionalJacobian<2, 3> Dpoint = std::nullopt) const {
    return Base::project(pw, Dpose, Dpoint);
  }

  /// project2 version for point at infinity
  Point2 project2(const Unit3& pw, OptionalJacobian<2, 6> Dpose = std::nullopt,
      OptionalJacobian<2, 2> Dpoint = std::nullopt) const {
    return Base::project(pw, Dpose, Dpoint);
  }
