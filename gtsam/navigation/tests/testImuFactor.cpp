...
TEST(ImuFactor, PreintegrationBaseMethods) {
  ...
  Matrix expectedH = numericalDerivative11<Vector9, Bias>(
      std::bind(&PreintegrationBase::biasCorrectedDelta, pim,
          std::placeholders::_1, std::nullopt), kZeroBias);
...
TEST(ImuFactor, ErrorAndJacobianWithBiases) {
  ...
  Matrix expectedH = numericalDerivative11<Vector9, Bias>(
      std::bind(&PreintegrationBase::biasCorrectedDelta, pim,
          std::placeholders::_1, std::nullopt), bias);
  EXPECT(assert_equal(expectedH, actualH));
...
TEST(ImuFactor, ErrorWithBiasesAndSensorBodyDisplacement) {
  ...
  pim.correctMeasurementsBySensorPose(measuredAcc, measuredOmega,
      std::nullopt, D_correctedAcc_measuredOmega, std::nullopt);
...
//  Matrix93 G1, G2;
//  Vector9 preint =
//      pim.updatedDeltaXij(measuredAcc, measuredOmega, dt, std::nullopt, G1, G2);
//
...
//          dt, std::nullopt, std::nullopt, std::nullopt), measuredAcc,
//      measuredOmega, 1e-6);
...
//  EXPECT(MonteCarlo(pim, NavState(x1, v1), bias, 0.1, std::nullopt, measuredAcc, measuredOmega,
