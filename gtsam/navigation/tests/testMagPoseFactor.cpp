...
TEST(MagPoseFactor, Constructors) {
  MagPoseFactor<Pose2> f2a(Symbol('X', 0), measured2, scale, dir2, bias2, model2, std::nullopt);
  MagPoseFactor<Pose3> f3a(Symbol('X', 0), measured3, scale, dir3, bias3, model3, std::nullopt);
...
TEST(MagPoseFactor, JacobianPose2) {
  ...
  MagPoseFactor<Pose2> f(Symbol('X', 0), measured2, scale, dir2, bias2, model2, std::nullopt);
...
TEST(MagPoseFactor, JacobianPose3) {
  ...
  // Error should be zero at the groundtruth pose.
  MagPoseFactor<Pose3> f(Symbol('X', 0), measured3, scale, dir3, bias3, model3, std::nullopt);
...
TEST(MagPoseFactor, body_P_sensor2) {
  ...
  CHECK(gtsam::assert_equal(Z_2x1, f.evaluateError(n_P2_b, H2), 1e-5));
  CHECK(gtsam::assert_equal(gtsam::numericalDerivative11<Vector, Pose2> //
      (std::bind(&MagPoseFactor<Pose2>::evaluateError, &f, std::placeholders::_1, std::nullopt), n_P2_b), H2, 1e-7));
}
...
TEST(MagPoseFactor, body_P_sensor3) {
  ...
  CHECK(gtsam::assert_equal(Z_3x1, f.evaluateError(n_P3_b, H3), 1e-5));
  CHECK(gtsam::assert_equal(gtsam::numericalDerivative11<Vector, Pose3> //
      (std::bind(&MagPoseFactor<Pose3>::evaluateError, &f, std::placeholders::_1, std::nullopt), n_P3_b), H3, 1e-7));
}
