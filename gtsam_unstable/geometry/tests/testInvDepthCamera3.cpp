...
TEST( InvDepthFactor, Dproject_pose)
{
  ...
  InvDepthCamera3<Cal3_S2> inv_camera(level_pose,K);
  Matrix actual;
  inv_camera.project(landmark, inv_depth, actual, std::nullopt, std::nullopt);
...
TEST( InvDepthFactor, Dproject_landmark)
{
  ...
  InvDepthCamera3<Cal3_S2> inv_camera(level_pose,K);
  Matrix actual;
  inv_camera.project(landmark, inv_depth, std::nullopt, actual, std::nullopt);
...
TEST( InvDepthFactor, Dproject_inv_depth)
{
  ...
  InvDepthCamera3<Cal3_S2> inv_camera(level_pose,K);
  Matrix actual;
  inv_camera.project(landmark, inv_depth, std::nullopt, std::nullopt, actual);
  EXPECT(assert_equal(expected,actual,1e-7));
}
