...
//  const bool debug = ISDEBUG("ConcurrentIncrementalFilter update");
  ...
  std::optional<FastMap<Key,int> > orderingConstraints = std::nullopt;
  ...
  std::optional<FastList<Key> > additionalKeys = std::nullopt;
  if(keysToMove && keysToMove->size() > 0) {
...
//  const bool debug = ISDEBUG("ConcurrentIncrementalFilter synchronize");
  ...
  ISAM2Result result = isam2_.update(currentSmootherSummarization, Values(), removeFactors, std::nullopt, noRelinKeys, std::nullopt, false);
