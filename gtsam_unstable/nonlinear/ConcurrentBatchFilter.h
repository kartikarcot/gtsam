...
public:
  ...
  virtual Result update(const NonlinearFactorGraph& newFactors = NonlinearFactorGraph(), const Values& newTheta = Values(),
      const std::optional<FastList<Key> >& keysToMove = std::nullopt, const std::optional< std::vector<size_t> >& removeFactorIndices = std::nullopt);
...
private:
  ...
  void reorder(const std::optional<FastList<Key> >& keysToMove = std::nullopt);
